...
describe('WebsocketController', function() {
	beforeEach(function() {
		...
		this.client = {
			...
			params: {},
			set: sinon.stub(),
			get(param, cb) { return cb(null, this.params[param]); },
			join: sinon.stub(),
			leave: sinon.stub()
		};
	...
	describe("joinProject", function() {
		describe("when authorised", function() {
			...
			it("should set the privilege level on the client", function() {
				return this.client.set.calledWith("privilege_level", this.privilegeLevel).should.equal(true);
			});

			it("should set the user's id on the client", function() {
				return this.client.set.calledWith("user_id", this.user._id).should.equal(true);
			});

			it("should set the user's email on the client", function() {
				return this.client.set.calledWith("email", this.user.email).should.equal(true);
			});

			it("should set the user's first_name on the client", function() {
				return this.client.set.calledWith("first_name", this.user.first_name).should.equal(true);
			});

			it("should set the user's last_name on the client", function() {
				return this.client.set.calledWith("last_name", this.user.last_name).should.equal(true);
			});

			it("should set the user's sign up date on the client", function() {
				return this.client.set.calledWith("signup_date", this.user.signUpDate).should.equal(true);
			});

			it("should set the user's login_count on the client", function() {
				return this.client.set.calledWith("login_count", this.user.loginCount).should.equal(true);
			});

			it("should set the connected time on the client", function() {
				return this.client.set.calledWith("connected_time", new Date()).should.equal(true);
			});

			it("should set the project_id on the client", function() {
				return this.client.set.calledWith("project_id", this.project_id).should.equal(true);
			});

			it("should set the project owner id on the client", function() {
				return this.client.set.calledWith("owner_id", this.owner_id).should.equal(true);
			});

			it("should set the is_restricted_user flag on the client", function() {
				return this.client.set.calledWith("is_restricted_user", this.isRestrictedUser).should.equal(true);
			});
		...
		describe("when the project is empty", function() {
			...
			it("should end clientTracking.clientDisconnected to the project room", function() {
				return this.WebsocketLoadBalancer.emitToRoom
					.calledWith(this.project_id, "clientTracking.clientDisconnected", this.client.id)
					.should.equal(true);
			});
	...
	describe("updateClientPosition", function() {
		...
		describe("with a logged in user", function() {
			beforeEach(function() {
				...
				this.client.get = (param, callback) => callback(null, this.clientParams[param]);
			...
			it("should send the  cursor data to the connected user manager", function(done){
				this.ConnectedUsersManager.updateUserPosition.calledWith(this.project_id, this.client.id, {
					_id: this.user_id,
					email: this.email,
