		...
		this.WebsocketLoadBalancer = SandboxedModule.require(modulePath, { requires: {
			...
			"./Utils": (this.Utils = {
				getClientAttributes: sinon.spy(
					(client, _attrs, callback) => callback(null, {is_restricted_user: !!client.__isRestricted}))
			})
		}
	});
		this.io = {};
			...
			beforeEach(function() {
				this.io.sockets = {
					clients: sinon.stub().returns([
						{id: 'client-id-1', emit: (this.emit1 = sinon.stub())},
						{id: 'client-id-2', emit: (this.emit2 = sinon.stub())},
						{id: 'client-id-1', emit: (this.emit3 = sinon.stub())} // duplicate client
					])
				};
			...
			beforeEach(function() {
				this.io.sockets = {
					clients: sinon.stub().returns([
						...
						{id: 'client-id-2', emit: (this.emit2 = sinon.stub())},
						{id: 'client-id-1', emit: (this.emit3 = sinon.stub())}, // duplicate client
						{id: 'client-id-4', emit: (this.emit4 = sinon.stub()), __isRestricted: true}
					])
				};
				const data = JSON.stringify({
					room_id: this.room_id,
