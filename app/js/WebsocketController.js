...
module.exports = (WebsocketController = {
	...
	joinProject(client, user, project_id, callback) {
		...
		return WebApiManager.joinProject(project_id, user, function(error, project, privilegeLevel, isRestrictedUser) {
			...
			client.set("owner_id", __guard__(project != null ? project.owner : undefined, x => x._id));
	...
	leaveProject(io, client, callback) {
		...
		return Utils.getClientAttributes(client, ["project_id", "user_id"], function(error, {project_id, user_id}) {
			if (error != null) { return callback(error); }
			...
			if ((project_id == null)) {
				logger.log({user_id, client_id: client.id}, "client leaving, not in project");
				return callback();
			}
	...
	joinDoc(client, doc_id, fromVersion, options, callback) {
		...
		return Utils.getClientAttributes(client, ["project_id", "user_id", "is_restricted_user"], function(error, {project_id, user_id, is_restricted_user}) {
			if (error != null) { return callback(error); }
	...
	updateClientPosition(client, cursorData, callback) {
		...
		return Utils.getClientAttributes(client, [
	...
	applyOtUpdate(client, doc_id, update, callback) {
		if (callback == null) { callback = function(error) {}; }
		return Utils.getClientAttributes(client, ["user_id", "project_id"], function(error, {user_id, project_id}) {
			if (error != null) { return callback(error); }
			if ((project_id == null)) { return callback(new Error("no project_id found on client")); }
